...
VibeHub 已支持从「本地文件夹」「远程链接」「手动新建」三种方式把 Skill 导入个人仓库。本功能新增第四种入口——**从 IDE 工具导入**：直接检索本机已安装 IDE 的**全局 Skill 目录**（Cursor / Codex 等），把其中的原生 Skill 导入到个人仓库。
...
- **同名 Skill 跨 IDE 重复**：按 IDE 分组分别展示；重复导入按 `name` upsert（后端 `NativeSkillStore` 既有语义），后导入覆盖。
...
- 三处改动文件 `ReadLints` 无报错。
- 端到端手测（需运行中的本地代理）：桌面壳启动后（`./build-desktop.ps1 -Quick -NoBe`）进入「个人 Skill 仓库 → + 新建 Skill → 从 IDE 工具导入」，确认能检索到本机 Cursor / Codex 等全局目录的 Skill 并成功导入。
