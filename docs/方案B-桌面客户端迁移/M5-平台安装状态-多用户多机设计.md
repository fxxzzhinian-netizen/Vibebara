...
>
> 上游依据：
> - 根目录 `方案B-桌面客户端迁移技术路径.md`（§2 三层架构、§3.4 鉴权与多租户、§5 接口边界）。
...
| 桌面壳改动 | `vibehub-device.json` 升级（`clientUuid` + 注册回写的 `deviceId`）+ 一个回写 IPC |
...
class Device(Base):
    ...
    # 本机持久 uuid（= M5-a vibehub-device.json 的 deviceId）；幂等再注册键，非身份凭证。
...
> **唯一约束选型**：`(user_id, client_uuid)` 而非 `client_uuid` 单列——因为**同一台机器可被多个 VibeHub 账号登录使用**（共享桌面），应产生各自独立的设备身份。
...
| 阶段 | 机制 |
| --- | --- |
| **产生（本机侧）** | 桌面主进程沿用 M5-a：`crypto.randomUUID()` 写 `<userData>/vibehub-device.json`，作为 **`clientUuid`**（本机持久、与登录用户无关）。 |
...
| **持久（回写本机）** | 前端把返回的 `device_id` 经桌面桥写回 `vibehub-device.json`（新增 `registeredDeviceId` 字段）；后续会话用规范 `device_id` 做上报维度键。 |
...
M5-a 现状（见 `M5-a-桌面壳骨架实施记录.md` §2.7/§5）：`deviceId.ts` 生成本机 uuid 存 `vibehub-device.json`，注入 `runtimeConfig.deviceId` 与桥 `deviceId`；前端 `VibehubRuntimeConfig.deviceId?` 仅透传不做业务判断。
...
sequenceDiagram
    ...
    Note over Shell: 启动：读/建 vibehub-device.json<br/>{ clientUuid, registeredDeviceId? }
    ...
    FE->>Shell: 桥 persistDeviceId(deviceId)  （回写 vibehub-device.json.registeredDeviceId）
    Note over FE: 之后所有平台安装态上报/查询用规范 deviceId
```

兼容与收敛细节：

- **字段语义平移**：`vibehub-device.json` 原 `deviceId` 字段（M5-a）→ 视为 `clientUuid`（迁移时读旧 `deviceId` 当 `clientUuid` 用，零数据丢失）。
- **回写 IPC**：M5-b 在桌面壳新增一个 `device:persist-id` IPC（主进程把 `registeredDeviceId` 落 `vibehub-device.json`），桥暴露 `persistDeviceId(id)`。这是桌面侧唯一新增点，属 M5-b 实施（本设计不改 `desktop/`）。
- **运行时字段**：`VibehubRuntimeConfig` 可在 M5-b 增可选 `clientUuid?`；`deviceId` 注入优先级 = `registeredDeviceId ?? clientUuid`，保证「未注册前也有占位、注册后即用正式 id」。前端编排在调用上报前确保 `deviceId` 已是注册后的规范值（未注册先注册）。
...
| **S4 桌面壳过渡** | `vibehub-device.json` 升 `{clientUuid, registeredDeviceId}`；`device:persist-id` IPC + 桥 `persistDeviceId`；运行时注入 `clientUuid` | `desktop/src/main/deviceId.ts`、`ipc.ts`、`preload/index.ts`、`runtimeConfig.ts` | — |
