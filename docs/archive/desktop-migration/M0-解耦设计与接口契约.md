...
>
> 上游依据：同目录 `方案B-桌面客户端迁移技术路径.md`（§4 改造埋点、§5 接口边界、附录 A 端点速查）。
>
...
- `installed_hash` = 本地代理对 `installPath` 算（本地算，权威）；二者内容不同（Store 含 `skill.config.yaml`+`SKILL.md`+资源，install 含构建出的 `SKILL.md`+资源），**本就不相等**，仅作各自基线。
...
- 云端鉴权：HMAC token，格式 `user_id.expire_ts.signature`（`auth_service._create_token` `:38-45`），secret = `settings.LLM_API_KEY or "vibebara-default-secret"`（`:20`），有效期 7 天。`get_current_user_id` 从 `Authorization: Bearer` 解析（`api/auth.py:21-35`）。
