...
class CursorAdapter(BaseAdapter):
    ...
    Handles communication via Cursor's extension API / MCP protocol.
    """
    ...
    async def translate_inbound(self, raw_message: Dict[str, Any]) -> UnifiedEvent:
        """Translate Cursor-native messages to unified format."""
        ...
        return UnifiedEvent(
            ...
            metadata={"original_type": msg_type, "tool": "cursor"},
        )

    async def translate_outbound(self, event: UnifiedEvent) -> Dict[str, Any]:
        """Translate unified event to Cursor-native format."""
        return {
            "type": self._map_event_to_cursor_type(event.type),
            "data": event.payload,
    ...
    async def send_to_tool(self, message: Dict[str, Any]) -> bool:
        # TODO: Implement actual Cursor API/MCP communication
        return True

    def _map_event_to_cursor_type(self, event_type: EventType) -> str:
        type_map = {
