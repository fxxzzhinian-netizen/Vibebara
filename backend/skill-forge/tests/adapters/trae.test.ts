...
describe("TraeAdapter", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "sf-trae-"));
    adapter = new TraeAdapter();
    config = UnifiedSkillSchema.parse({
      name: "test-skill",
      description: "A test skill for Trae",
  ...
  it("generates SKILL.md with name + description frontmatter", async () => {
    ...
    expect(frontmatter["description"]).toBe("A test skill for Trae");
  ...
  it("returns a global Trae deploy directory under ~/.trae or ~/.trae-cn", () => {
    const deployDir = adapter.getDeployDir();
    expect(deployDir).toBe(resolveTraeSkillsDir());
    const intl = path.join(os.homedir(), ".trae", "skills");
    const cn = path.join(os.homedir(), ".trae-cn", "skills");
    expect([intl, cn]).toContain(deployDir);
  });

  it("applies trae target overrides", async () => {
    const overridden = UnifiedSkillSchema.parse({
      ...config,
      targets: { trae: { description: "Overridden for Trae" } },
    });
    ...
    expect(frontmatter["description"]).toBe("Overridden for Trae");
  });
  ...
  it("copies scripts/references/assets resources", async () => {
    const srcSkill = await fs.mkdtemp(path.join(os.tmpdir(), "sf-trae-src-"));
